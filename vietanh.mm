...
// Tự khai báo trực tiếp nguyên mẫu hàm ImGui Metal để tránh lỗi xung đột phiên bản header
extern "C" {
    bool ImGui_ImplMetal_Init(id<MTLDevice> device);
    void ImGui_ImplMetal_Shutdown();
    void ImGui_ImplMetal_NewFrame(MTLRenderPassDescriptor* renderPassDescriptor);
    void ImGui_ImplMetal_RenderDrawData(ImDrawData* draw_data, id<MTLCommandBuffer> commandBuffer, id<MTLCommandEncoder> commandEncoder);
}
...
static void hk_mtkView_draw(id self, SEL _cmd) {
    ...
    if (g_Initialized) {
        ...
        if (renderPassDescriptor && drawable) {
            ...
            ImGui_ImplMetal_RenderDrawData(draw_data, commandBuffer, (id<MTLCommandEncoder>)commandEncoder);
